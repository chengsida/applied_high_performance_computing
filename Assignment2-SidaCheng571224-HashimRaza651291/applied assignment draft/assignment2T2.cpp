...
const double	roh		= 8954.0;
const double	cap		= 380.0;
const double	k		= 386.0;
const double    Qcpu		= 40000.0;
const double    h		= 100.0;
const double    Tair		= 300.0;
const int		N_t			= static_cast<int> ((t_max-t_min)/Delta_t+1);

// Function declarations
void	readData(char* filename, double**& Points, int**& Faces, int**& Elements, Boundary*& Boundaries, int& N_p, int& N_f, int& N_e, int& N_b);
void	writeData(double* T, double**& Points, int**& Elements, int& N_p, int& N_e, int l);
void	assembleSystem(SparseMatrix& M, SparseMatrix& K, double* s, double* T, double** Points, int** Faces, int** Elements, Boundary* Boundaries, int N_p, int N_f, int N_e, int N_b);
void	solve(SparseMatrix& A, double* T, double* b);
...
int     main(int argc, char** argv)
{
	...
	{
		readData(argv[1], Points, Faces, Elements, Boundaries, N_p, N_f, N_e, N_b);
	}
	// Allocate arrays
    double*			T			= new double[N_p];
    ...
    assembleSystem(M, K, s, T, Points, Faces, Elements, Boundaries, N_p, N_f, N_e, N_b);
	...
	writeData(T, Points, Elements, N_p, N_e, 0);
	...
	for(int l=0; l<N_t-1; l++)
	{
		...
		solve(A, T, b);
        	...
        	if(l%10==0)
            {
			 writeData(T, Points, Elements, N_p, N_e, l);
		    }
	}
...
{	
    ...
    sprintf(fileName, "VTK/BOX_%02d_%04d.vtk", l);
    ...
    for(int p=0; p<N_p; p++)
    {
        file << setw(6) << setprecision(5) << Points[p][0] << "\t" << Points[p][1] << "\t" << Points[p][2] << "\t" << T[p] << endl;
    }
    
    file << "CELLS " << N_e << " " << 4*N_e << endl;
    for(int e=0; e<N_e; e++)
    {
        file << "3\t" << Elements[e][0] << "\t" << Elements[e][1] << "\t" << Elements[e][2] << "\t" << Elements[e][3] << endl;
    }
...
void	assembleSystem(SparseMatrix& M, SparseMatrix& K, double* s, double* T, double** Points, int** Faces, int** Elements, Boundary* Boundaries, int N_p, int N_f, int N_e, int N_b)
{
    ...
    for(int e=0; e<N_e; e++)
	{
		...
		Omega[e]    = Omega[e]    = fabs(  x[0]*y[1]*z[2] - x[0]*y[2]*z[1] - x[1]*y[0]*z[2] 
		...
		if		(Boundaries[b].type_=="neumann")
        {
			for(int f=0; f<Boundaries[b].N_; f++)
			{
				for(int p=0; p<3; p++)
				{
					...
					m			= Nodes[p];
					s[m]		+= Qcpu*Gamma[Boundaries[b].indices_[f]]/3.0;
				}
			}
		}
		else if	(Boundaries[b].type_=="robin")
		{
			for(int f=0; f<Boundaries[b].N_; f++)
			{
				for(int p=0; p<3;p++)
				{
					Nodes[p]	= Faces[Boundaries[b].indices_[f]][p];
					m			= Nodes[p];
					s[m]		+= h*Tair*Gamma[Boundaries[b].indices_[f]]/3.0;
						for(int q=0;q<3;q++)
						{
							Nodes[q]	= Faces[Boundaries[b].indices_[f]][q];
                            n=Nodes[q];
							K(m,n)-= K_e[p][q]*Gamma[Boundaries[b].indices_[f]]*h/12.0;
                        }
				}
