...
int     main(int argc, char** argv)
{
	...
	int           i, j ,l ; 
	double        x, y;
	double wtime = omp_get_wtime();                      //Monitor the starting time
    ...
    cout << "Simulation took " << wtime << " seconds with " << N_Threads << " threads" << endl;
...
void    f(double** vx, double** vy, double** h, double** kx, double** ky, double** kh)
{
	int i, j, x_a, x_b,x_c,x_d,y_e,y_f,y_g,y_h;
...
void writeData(int k, double** h, double x_min, double y_min, double delta_x, double delta_y, int N_x, int N_y)
{
	...
	char fileName[128];
	fstream file;
	sprintf(fileName, "waterequationOpenMP_%05d.csv", k);
